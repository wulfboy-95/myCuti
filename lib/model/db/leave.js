    ...
    var Leave = sequelize.define("Leave", {
      ...
      classMethods : {
        associate : function( models ){
          ...
          Leave.belongsTo(models.User, { as : 'replacer',foreignKey : 'duty_replacer' });
...
is_duty_none : function () {
  return this.duty_status === Leave.duty_none();
},

is_duty_unconfirmed :  function () {
  return this.duty_status === Leave.duty_unconfirmed();
},

is_duty_confirmed :  function () {
  return this.duty_status === Leave.duty_confirmed();
},

is_duty_denied :  function () {
  return this.duty_status === Leave.duty_denied();
},
...
// Leave is treated as "approved" one if it is in approved status
...
get_deducted_days : function(args) {
  ...
  // Because we currently in synchronous code we have to rely on cached value
  // rather then fetching it here, and prey that whoever called current
  // method made sure that the cache is populated
  ...
  leave_days = _.filter(
    _.map(this.get_days(), function(leave_day){
      ...
      // If it happened that current leave day is from the year current
...
promise_to_confirm_duty_replacement : function() {
  var self = this;
  if (self.is_duty_unconfirmed()) self.duty_status = Leave.duty_confirmed();
  console.log("Duty Confirmed");
  return self.save();
},

promise_to_deny_duty_replacement : function() {
  var self = this;
  if (self.is_duty_unconfirmed()) self.duty_status = Leave.duty_denied();
  console.log("Duty Denied");
  return self.save();
},
        },
    });
