...
router.post('/bookleave/', upload.single("attachment"), function(req, res){
    ...
    Promise.join (
      ...
      (users, company, valid_attributes) => {
        ...
        check_attr(company.is_mode_readonly_holidays(), "Company account is locked and new Timeoff requests could not be added. Please contact administration.",'Company is in "Read-only holidays" mode');
        ...
        if (req.file) {
          ...
          valid_attributes.attachment = newpath; // adding the attachment here instead of in validator
        }
