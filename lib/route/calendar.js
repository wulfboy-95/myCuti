...
function check_attr(cond, msg1, msg2) {
  if (cond) {
    req.session.flash_error(msg1);
...
router.post('/bookleave/', upload.single("attachment"), function(req, res){
    ...
    Promise.join (
      ...
      (users, company, valid_attributes) => {
        ...
        check_attr(!employee,'Incorrect employee', 'Got validation errors');
        check_attr(!leave_type,'Incorrect leave type','Got validation errors');
        check_attr(company.is_mode_readonly_holidays(), "Company account is locked and new time off requests could not be added. Please contact administration.",'Company is in "Read-only holidays" mode');
        ...
        if (req.file) {
          ...
          valid_attributes.attachment = newpath;
        }
...
router.get('/', function(req, res) {
  ...
  Promise.join(
    ...
    function(calendar, company, user, supervisors, user_allowance, my_duties){
      let
