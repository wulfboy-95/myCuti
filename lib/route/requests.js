...
router.get('/', function(req, res){
    ...
    Promise.join(
        ...
        req.user.promise_duty_replacements(),
        function(my_leaves_grouped, to_be_approved_leaves, my_duties){
            res.render('requests',{
...
//Duty replacement methods
router.post('/dutyconfirm/', function(req, res) {
  var leave_id = validator.trim( req.param('duty') );
  console.log(leave_id);
  Promise.try(function() {return req.user.promise_duty_replacements();})
  .then(function(leaves) {
    var leave_duty_to_confirm = _.find(leaves, function(leave) {
      return String(leave.id) === String(leave_id);
    });
    if (!leave_duty_to_confirm) {
      throw new Error('Could not find leave for replacement duty.');
    }
    return Promise.resolve(leave_duty_to_confirm);
  })
  .then((leave_duty_to_confirm) => {
    console.log(leave_duty_to_confirm);
    return leave_duty_to_confirm.promise_to_confirm_duty_replacement();
  })
  .then(res.redirect_with_session('../'));
});

router.post('/dutydeny/', function(req, res) {
  var leave_id = validator.trim( req.param('duty') );
  Promise.try(function() {return req.user.promise_duty_replacements();})
  .then(function(leaves) {
    var leave_duty_to_deny = _.find(leaves, function(leave) {
      return String(leave.id) === String(leave_id);
    });
    if (!leave_duty_to_deny) {
      throw new Error('Could not find leave for replacement duty.');
    }
    return Promise.resolve(leave_duty_to_deny);
  })
  .then((leave_duty_to_deny) => {
    return leave_duty_to_deny.promise_to_deny_duty_replacement();
  })
  .then(res.redirect_with_session('../'));
});
...
router.post('/upload/', upload.single('attachment'), function(req,res) {
  ...
  .then(function(leave_to_upload) {
    var newname = leave_to_upload.from_date +"-"+ req.file.originalname;
