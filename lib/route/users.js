...
router.post('/add/', function(req, res){
  ...
  .then(() => UserImporter.add_user({
    ...
    position        : new_user_attributes.position,
    staff_no        : new_user_attributes.staff_no,
    address         : new_user_attributes.address,
    tel_no          : new_user_attributes.tel_no,
...
router.post('/import-sample/', function(req, res){
  ...
  req.user
    ...
    .then(company => {
      ...
      content.unshift( ['email', 'name', 'lastname', 'position', 'staff_no', 'department', 'address', 'tel_no'] );
...
router.get('/', function(req, res) {
    ...
    // Now only basic simple objects to be sent over to templates,
...
function get_and_validate_user_parameters(args) {
    ...
    var name     = validator.trim(req.param('name')),
        ...
        position          = validator.trim(req.param('position')),
        staff_no          = validator.trim(req.param('staff_no')),
        tel_no            = validator.trim(req.param('tel_no')),
        address   = validator.trim(req.param('address')),
